...
- `ablation/consistency_feature_rules.md`：跨层一致性特征构造规则的自然语言说明，可直接用于论文写作和答辩解释。
...
- 将 38 个跨层一致性特征的构造规则整理为自然语言说明，便于论文和答辩复用。
...
详细实验说明见 `ablation/README.md`。一致性特征构造规则见 `ablation/consistency_feature_rules.md`。论文和答辩图表说明见 `ablation/figure_guide.md`。

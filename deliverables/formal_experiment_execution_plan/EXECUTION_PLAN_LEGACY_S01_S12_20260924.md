...
计划版本：`formal-experiment-execution-plan-v1-20260923`。编制日期：2026-09-23。当前进度：**S01–S05 已完成；S03-R v2、S05-R 外部审查通过；S06 已完成并等待审查；S07–S12 未执行。** 原始编制快照保留在第 2 节；最新执行事实见 `EXECUTION_STATUS.json`。
...
**S05 v2 冻结记录：**精确矩阵、评估配置键、Fig.6 占位冲突处理、诊断与自然 MTC 分支范围见 [S05 版本化冻结说明](S05_PROTOCOL_FREEZE_v2.md)。以下原设计条文保留；未来运行以已冻结 JSON、v2 风险合同及单步授权为准。
...
### S05 执行更新（2026-09-24，北京时间）

按用户指令先提交推送 S04：`f00e699d2763afed5369774f01e5a0f632d8f5c6`，远端 main 与本地 HEAD 相同，74 个明确文件交付，270 个无关工作区条目保留。历史报告的推送前文字不重写。

随后仅执行运行前静态冻结，协议 `formal-manipulation-protocol-v2`、研究 `formal-manipulation-v1`，继续采用 S03-R A/B/C v2 与 S04 family-or-v2，7 候选/5 家族/阈值1。独立配置 `hybridguard_agent/config/formal_manipulation_protocol_v2/`，产物 `hybridguard_agent/artifacts/formal_manipulation_v1_20260923/05_freeze/`。

固定13唯一风险变体：主216阶段共2808单元，低证据45+不完整1仅S10单一条件描述，共2854唯一计划单元；262阶段全部有去向。S06/S07/S08/S10首次单元分别432/1512/864/46，跨步相同条件复用。攻击/pre/post与准入三态分母各54；时间对照阴性分母0，FPR与对应Fig.6操作点关闭。没有登记跨包共同批次键，因此不构造配对差分，保留分层展示。

E4四诊断精确定义与480计划项、49合成输入、E6固定24阶段的1152重复计划均已冻结。S04 Fig.6占位与总计划差异明确修订；评估侧补齐空configuration_id的原config_id映射，S01事实和S02原输入/索引不变。自然MTC仅材料分类及P6保存复用，新运行需另行冻结；不在本版暗增矩阵。

14项聚焦静态测试和生成后有限manifest/schema/集合检查通过。135文件有限绑定包含44参与源码与25运行配置/资源；无全库哈希。282个登记历史文件未改。真实/合成新预测0、规则/门控调用0、真实指标NOT_EVALUATED、LLM0。S05 DONE/PASS，停止等待审查或S06明确授权；未自动提交推送S05，未执行S06–S12。

### S05-R 打包修订更新（2026-09-24，北京时间）

审查基线 `595d30d690713f6ee800a7825cfe8199cee97565`。旧 `05_freeze` 的 manifest 未登记 runtime_cards 必需的三份来源 JSON；旧验收是已登记文件的静态检查，不能证明独立运行资源完备。保留旧目录、验收及上述推送前时间点记录，本次另建 `05_freeze_r1` 与 `formal_manipulation_protocol_v2_r1`，采用独立 `formal-manipulation-freeze-r1` 标识和新 digest。v2 schema、relation/risk 合同、策略名称和实验语义不变。

修订说明见 `S05_R_PACKAGING_REVISION_r1.md`。新增显式资源清单及摘要、样本读取前的缺件预检、独立快照合成完整链与遗漏回归。两个方法各验证一致/冲突输入，原 Verifier 和风险 Verifier 均通过；三份缺件在 CLI/API 启动阶段分别拒绝。实验输入、标签、准入、关联、规则/候选/家族、门控/OR/阈值、单位集合、分母、指标/图表及暴露历史保持不变；具体实测清单数量和日志见新快照 STEP_REPORT/VALIDATION，不硬编码文件总数。

后续 S06 仅在审查通过且另获授权后，绑定修订快照自身源码、内部 v2 配置/策略与新 protocol_digest；不能从旧包或主工作区回退。S05-R 合成输出独立保存、不计入 S06；真实预测/真实性能仍 NOT_EVALUATED，时间对照仍 UNKNOWN、无 FPR 资格。本次停止等待审查，不自动进入 S06，不自动提交或推送。


### S06 执行更新（2026-09-24，北京时间）

审查及运行起止 HEAD 为 `f3f934d0f93d72e16b3d97e4e390796065e5fff8`。本次用户明确接受 S05-R 并授权 S06；旧报告的推送前文字和冻结静态计划中的历史未授权状态原样保留。实际执行以修订快照 `05_freeze_r1`、`formal-manipulation-freeze-r1` 及 digest `9a6cb92a5d973a42d230305e176ab9ef824bca2e8aae045dafed515ce61c0f19` 为准，不能使用初始章节的旧快照或拟议 CLI 示例。

运行前完成冻结与资源预检，在独立临时目录复制并绑定快照源码/资源，仅补建 persistence 所需两个空运行目录；未改任何冻结文件。由本次授权另生成 S04 v2 runnable job。按最终冻结清单执行两个方法 final_v3_v2 / legacy19_v2，各 216 阶段、均为 App177 / SRC-111，共 432 单元；初始章节中的拟议策略数量不用于本次运行。使用原完整规则链、runtime_cards、原 Verifier 及独立 v2 风险策略/Verifier，不以单关系 probe 替代。

432/432 单元完整且唯一，22,896 条规则事件、432 条 runtime 全部保存；失败与弃判均为 0、空表保留。先关闭预测并对账，再由独立进程读取评估事实。两个方法的攻击检出均 0/54，clean_pre/post 报警各 0/54 observed，精确 010 均 0/54；至少一个候选家族可评估的决策覆盖均 216/216，但不代表全家族覆盖。时间对照仍 UNKNOWN，54 时点/18 midpoints 每方法仅描述，FPR 分母 0、值 null。

上述为保存的科学负结果，不能用 clean 零报警或 100% 决策覆盖包装检测能力。各方法 36/54 攻击阶段有原非候选关系反例，但没有按标签或结果晋级；规则、角色、门控、家族 OR、阈值、标签及分组不变。来源 H/C 无候选与 O_u/E 重叠仍是事前结构限制，本步没有来源消融结论。

23 项保存结果一致性检查通过，不重跑检测器或合成验收；只读检查器的报警枚举修正单独留档，不涉及预测或指标更改。产物位于 `hybridguard_agent/artifacts/formal_manipulation_v1_20260923/06_e1/`，含完整长表、40 行逐配置表现/原因、逐单元未检出说明、run_manifest、VALIDATION 和 STEP_REPORT。S06 DONE/PASS 仅为执行和记录验收，等待外部审查；S07 及后续仍未授权，未自动提交或推送。

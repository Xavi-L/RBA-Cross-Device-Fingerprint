...
计划版本：`formal-experiment-execution-plan-v1-20260923`。编制日期：2026-09-23。状态：**仅完成阅读、静态核对与计划；S01–S12 均未执行、均待逐步授权。**
...
已核对仓库、攻击仓库和祖先目录，未找到适用的实体 `AGENTS.md`；本轮遵循用户在会话中提供的 AGENTS 约束。未运行实验、检测器、材料校验器、测试、攻击工具、训练或模型调用，未启动服务，未修改业务代码、规则、数据及历史结果。仅按用户明确要求更新攻击仓库，并新建两份计划文件。资料包已经解压存在，无需另行解压。
...
本轮仅生成本文件与 `EXECUTION_STATUS.json`；没有任何S步骤完成。进度文件保存两个仓库版本、资料包基线、工作区原状、逐步依赖/预期产物/验收/阻塞和`next_authorization_step=S01`。

**第一个建议指令：`执行 S01`。** 它会复验现有材料校验器、建立独立事实准入与环境关联台账、列出真正缺件；不会运行检测策略、正式实验、攻击工具、训练、模型、采集服务，也不会修改原始标签或自动进入S02。

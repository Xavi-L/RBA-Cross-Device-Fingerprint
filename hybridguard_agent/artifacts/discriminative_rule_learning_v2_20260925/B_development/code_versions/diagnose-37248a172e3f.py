...
def after_trials():
    ...
    collisions, summaries, candidate_stats, changes, integrity = [], [], [], [], []
    for rep in ('S_FLAT','J0'):
        predictions[rep] = {}
        for f in folds():
            d = OUT/'trials'/('V2-A__'+rep+'__'+f['fold_id']+'__attempt01')
            job = {'job_id': 'V2-A__'+rep+'__'+f['fold_id'], 'fold_id': f['fold_id'],
                   'representation': rep, 'outer_test_ids': f['outer_test']}
            inspected = inspect_attempt(d, job)
            rows = inspected['predictions']
            predictions[rep].update({r['opaque_id']:r for r in rows})
            integrity.append(dict(inspected['audit'], representation=rep, fold_id=f['fold_id']))
            m, training = inspected['model'], inspected['training']
            if m is None:
                model_rows.append({'representation': rep, 'fold_id': f['fold_id'], 'model_id': None,
                    'status': inspected['audit']['status'], 'complexity': None, 'clauses': None,
                    'path': str(d/'model.json'), 'structure_statistics': inspected['audit']['structure_statistics']})
                continue
    ...
    write(OUT/'trial_integrity_summary.json',integrity)

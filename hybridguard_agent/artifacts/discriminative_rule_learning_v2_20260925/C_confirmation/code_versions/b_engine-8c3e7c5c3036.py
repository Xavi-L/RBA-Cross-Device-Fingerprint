...
def load_initial(path):
    ...
    if schema == 'v2-c-model-v1':
        from .c_engine import load_model as load_c
        return load_c(path)
...
def fit_b(access, defs, spec, binding, relation_rows=None):
    return _fit_unified(access, defs, spec, binding, relation_rows, BModel, 'V2-B')


def _fit_unified(access, defs, spec, binding, relation_rows, model_type, phase):
    """Shared mathematical stages; phase entry points own authorization/identity."""
    ...
    view = {'view_id':phase+':'+spec['representation'], 'representation':spec['representation'],
            'kind':phase.replace('-','_')+'_UNIFIED','base_representation':spec['base_representation'],
    ...
    audit = dict(outcome, train_ids=list(access.ids), train_expected_n=len(access.ids),
                 train_consumed_n=len(access.rows), fit_scope=phase.replace('-','_')+'_EXACT_AUTHORIZED_TRAIN_ONLY',
    ...
    model = model_type(RuleModel(spec['method'],status,tuple(selected),tuple(a for a in atoms if a.atom_id in selected_ids),
...
def predict_current(model, oid, raw, relation_input=None):
    ...
    try:
        ...
        result.update(decision='FAILED',failure_reason=model.binding['phase'].replace('-','_')+'_TRANSFORM:'+str(exc))
        return result

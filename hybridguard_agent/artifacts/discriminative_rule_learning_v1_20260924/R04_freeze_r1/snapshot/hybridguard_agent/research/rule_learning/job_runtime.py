...
from .baselines import restrict_single_surface, single_surface_definitions
...
class FrozenSnapshot:
    def __init__(self, root):
        ...
        # Metadata only: the complete audit catalog is preserved, while each
        # formal single-layer job must bind exactly the R02 admitted columns.
        for ref in {j["definitions_ref"] for j in self.models if j["input_view"] != "core"}:
            definitions = read_json(self.check_resource(ref))
            expected = {s: [a.atom_id for a in single_surface_definitions(s)]
                        for s in ("native84", "app_web67", "host26")}
            if definitions.get("single_surface_allowlists") != expected:
                raise ValueError("FORMAL_SINGLE_SURFACE_ALLOWLIST_DIFFERS_FROM_R02")
...
class JobContext:
    ...
    def train(self):
        ...
        view = copy.deepcopy(definitions["view"])
        # Discard even same-surface audit/deployment columns before fitting.
        if self.job["input_view"] != "core":
            surface = self.job["input_view"]
            allowed = definitions["single_surface_allowlists"][surface]
            atoms = restrict_single_surface(atoms, surface, allowed)
            view["single_surface_allowlist"] = {"surface": surface, "atom_ids": allowed,
                                                "definitions_ref": self.job["definitions_ref"]}
            self.event("SINGLE_SURFACE_ALLOWLIST_APPLIED", surface=surface, atom_ids=allowed)
        ...
        return JobTrainingAccess(_ISSUER, self.job, rows, meta, atoms, view, self.lineage())

"""Build a NEW R04 freeze and run only its built-in synthetic acceptance."""
...
from hybridguard_agent.research.rule_learning.freeze import build


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--out', type=Path, default=STUDY / 'R04_freeze')
    args = parser.parse_args()
    protocol = build(args.out)
    # Kept after verification so recorded absolute module paths remain inspectable.
    independent = Path(tempfile.mkdtemp(prefix='hybridguard-r04-isolated-'))
    ...
    print(json.dumps({'status': 'PASS_R04_FREEZE_SYNTHETIC_ONLY', 'output': str(args.out),
                      'R05_counts': protocol['R05_counts'], 'real_fits': 0, 'real_predictions': 0}))

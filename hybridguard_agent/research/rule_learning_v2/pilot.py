...
def preserve_failed_attempt(dest, job, reason):
    """A partially written prediction file is evidence, never a successful receipt."""
    failure = {'state': 'FAILED', 'failure_reason': reason, 'actual_fit_invocations': 1}
    write(dest / 'worker_failure.json', failure)
    write_lines(dest / 'failure_predictions.jsonl', failed_rows(job, reason))
    return failure


def attempt_predictions(dest):
    if (dest / 'worker_failure.json').exists():
        return lines(dest / 'failure_predictions.jsonl')
    return lines(dest / 'predictions.jsonl')
    ...
    try:
        ...
        for job in spec['jobs']:
            ...
            try:
                ...
                receipt = preserve_failed_attempt(dest, job, str(exc))

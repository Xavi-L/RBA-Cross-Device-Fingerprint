...
def fit(access, batch, method="GREEDY_OR", operating_point="OP05"):
    ...
    b = binding()
    metadata = {k: b[k] for k in ("study_version", "protocol_digest", "candidate_version", "input_manifest_ref")}
    metadata.update(fold_id=access.fold_id, split_id="SYNTHETIC", operating_point=operating_point)
    metadata.update(protocol_input_manifest_ref=metadata["input_manifest_ref"],
                    input_manifest_ref="hybridguard_agent/research/rule_learning/synthetic.py#" + access.fixture_name,
    ...
    audit = dict(outcome, train_ids=list(batch.ids), train_expected_n=len(batch.ids), train_consumed_n=len(batch.records),
                 fit_scope="SYNTHETIC_ONLY", fixture_name=access.fixture_name, elapsed_seconds=time.monotonic() - start,

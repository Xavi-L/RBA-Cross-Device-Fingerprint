...
def evaluate(expected_ids, predictions, metadata, *, evaluation_role="SYNTHETIC_TEST", strata=True):
    ...
    for oid in missing:
        index[oid] = {"opaque_id": oid, "decision": "FAILED", "failure_reason": "MISSING_EXPECTED_PREDICTION",
                      "selected_atoms_available": 0, "selected_atoms_expected": 0,
                      "clauses_defined": 0, "clauses_expected": 0}
    ...
    for name, numerator_key, denominator_key in (("atom_coverage", "selected_atoms_available", "selected_atoms_expected"),
        ...
        metrics[name] = rate(sum(index[i][numerator_key] for i in ids), sum(index[i][denominator_key] for i in ids))
        if missing and not (atom_sizes if name == "atom_coverage" else clause_sizes):
            metrics[name].update(status="NOT_EVALUABLE", reason="MISSING_MODEL_EXPECTED_CELL_COUNTS")

...
def aggregate_oof(expected_models, predictions, model_receipts, metadata):
    ...
    for key, jobs in sorted(groups.items()):
            ...
            for oid in j["outer_test_ids"]:
                ...
                if r is None:
                    ...
                    r = {"opaque_id": oid, "decision": "FAILED", "failure_reason": "MISSING_EXPECTED_OOF_UNIT_OR_MODEL",
                         "selected_atoms_available": 0, "selected_atoms_expected": (receipt or {}).get("atoms", 0),
                         "clauses_defined": 0, "clauses_expected": (receipt or {}).get("clauses", 0)}
        ...
        if any(f["receipt"] is None for f in folds):
            for name in ("atom_coverage", "clause_coverage"):
                report["metrics"][name].update(status="NOT_EVALUABLE", reason="MISSING_MODEL_EXPECTED_CELL_COUNTS",
